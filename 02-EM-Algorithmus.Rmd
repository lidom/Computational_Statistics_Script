...
Der EM Algorithmus wird häufig verwendet, um komplizierte Maximum Likelihood Schätzprobleme zu vereinfachen. Wir stellen den Algorithmus zur Schätzung von Gaußschen Mischverteilungen vor, da der EM-Algorithmus hier wohl seine häufigste Anwendung hat. Bereits die originale Arbeit zum EM-Algorithmus [@Dempster_1977] beschäftigt sich mit solchen Mischverteilungen.
...
> **Probleme mit Singularitäten:** Für echte GMMs ($G>1$) treten während einer numerischen Maximierung sehr leicht Probleme mit Singularitäten auf. Dies geschieht immer dann, wenn eine der Normalverteilungskomponenten versucht den ganzen Datensatz $\mathbf{x}$ zu beschreiben und die andere(n) Normalverteilungskomponente(n) versuchen lediglich einzelne Datenpunkte zu beschreiben. Eine Gaußsche Dichtefunktion $f_g$, die sich um einen einzigen Datenpunkt $x_i$ konzentriert (d.h. $\mu_g=x_i$ und $\sigma_g\to 0$) wird dabei sehr große Werte annehmen (d.h. $f_g(x_i)\to\infty$) und so die Log-Likelihood auf unerwünschte maximieren (siehe Abbildung \@ref(fig:dirac)). Solche trivialen Maximierungslösungen resultieren i.d.R. in unplausiblen Schätzergebnissen. 



```{r dirac, animation.hook="gifski", interval=0.1, include=knitr::is_html_output(), fig.align="center", echo=FALSE, fig.cap="Normalverteilung $f_g$ mit $\\mu_g=x_i$ für $\\sigma_g\\to 0$."}
np  <- 1000
rep <- 30
xxd <- seq(-3, 3, length.out = np)
sd_v <- seq(.5,0.05, len=rep)
for(i in 1:length(sd_v)){
  plot(x=xxd, y=dnorm(xxd, 0, sd_v[i]), type="l", ylim=c(0,max(dnorm(xxd, 0, sd_v[rep]))), xlim = c(-1.5,1.5),
...
<!-- include=knitr::is_html_output(), fig.align="center", animation.hook="gifski", echo=FALSE, fig.cap="Normalverteilung für $\\sigma_g\\to 0$." -->

<!-- ```{r , fig.show="animate", animation.hook="gifski"} -->

<!-- # knitr::include_graphics("images/Dirac_function_approximation.gif", ) -->
...
Der EM Algorithmus ermöglicht es Maximum Likelihood Probleme zu vereinfachen, indem man die Daten durch nicht beobachtete („latente“) Variablen vervollständigt. Dieses Prinzip ist der eigentliche wahre Beitrag des EM-Algorithmus. Es ermöglicht die Lösung verschiedener Maximum Likelihood Probleme - wie bleiben aber zunächst bei der Schätzung von GMMs. 
...
=\sum_{i=1}^n\ln\left(\sum_{g=1}^G\pi_g\phi_{\mu_g\sigma_g}(x_i)\right)
$$
direkt zu maximieren. Die $\log(\sum_{g=1}^G[\dots])$-Konstruktion macht einem hier das Leben schwer.

In unseren Pinguin-Daten gibt zwei Gruppen ($g\in\{1,2\}$). Es gäbe also im Prinzip $G$-dimensionale Zuordnungsvektoren $(z_{i1},z_{i2})$ mit 
$$
(z_{i1},z_{i2})=
\left\{\begin{array}{ll}
...
> **Beachte:**  Für jeden Datenpunkt $i$ (jeder Pinguin $i$) gibt es nur **eine** Gruppe (daher $\sum_{g=1}^Gz_{ig}=1$). Dies ist eine wichtige Restriktion von GMMs, welche bei den Pinguindaten unproblematisch ist, in Anwendungen mit hirarchischen Gruppierungen ist dies aber evtl. problematisch. 
...
$$
geben die Anteile der Einzelverteilungen $f_g$ an der Gesamtverteilung $f$ an. Im Schnitt kommen $\pi_g\cdot 100\%$ der Datenpunkte von Verteilung $f_g$, $g=1,\dots,G$. Somit ist die (latente) Zuordnung $z_{ig}$ eine Realisation der Zufallsvariablen $Z_{ig}$ mit Wahrscheinlichkeitsfunktion
$$
P(Z_{ig}=1)=\pi_g
$$
Wegen der Bedingung $\sum_{g=1}^Gz_{ig}=1$, sind alle anderen Zuordnungen gleich null. 
...
**A-priori-Wahrscheinlichkeit $\pi_g$:** Man bezeichnet die Wahrscheinlichkeiten $\pi_1,\dots,\pi_G$ als die *a-priori-Wahrscheinlichkeiten*. Wenn wir nichts über die Flossenlänge von Pinguin $i$ wissen, dann bleiben uns nur die a-priori-Wahrscheinlichkeiten: Mit Wahrscheinlichkeit $P(Z_{ig}=1)=\pi_g$ gehört Pinguin $i$ zu Gruppe $g$. 


**A-posteriori-Wahrscheinlichkeit $\;p_{ig}$:** Falls wir die Flossenlänge $x_i$ von Pinguin $i$ erfahren, können wir die a-priori-Wahrscheinlichkeiten mit Hilfe des Satzes von Bayes aktualisieren. Dies führt dann zur *a-posteriori-Wahrscheinlichkeit*: Mit Wahrscheinlichkeit $p_{ig}=P(Z_{ig=1}|X_i=x_i)$  gehört ein Pinguin $i$ mit Flossenlänge $x_i$ zu Gruppe $g$.
...
Der folgende EM-Algorithmus unterscheidet sich lediglich in der Notation von der oben eingeführten Version. Hier wird es klar, warum es ein **Expectation**-Maximization Algorithmus ist: 
<br>
...
2. Für $r=1,2,\dots$
    ...
    - <span style="color:#FF5733">**(Expectation)** </span> Berechne:        
    $$p_{ig}^{(r)}=E_{\theta^{(r-1)}}\left(Z_{ig}\left|X_i=x_i\right.\right)=\frac{\pi_g^{(r-1)}\phi_{\mu^{(r-1)}_g\sigma_g^{(r-1)}}(x_i)}{f_G(x_i|\boldsymbol{\pi}^{(r-1)},\boldsymbol{\mu}^{(r-1)},\boldsymbol{\sigma}^{(r-1)})}$$
...
Hätten wir neben den Datenpunkten $\mathbf{x}=(x_1,\dots,x_n)$ die Gruppenzuordnungen $\mathbf{z}=(z_{11},\dots,z_{nG})$
...
&=\prod_{i=1}^n\prod_{g=1}^G\left(\pi_gf_g(x_i|\mu_g,\sigma_g)\right)^{z_{ig}}\\[2ex]
\tilde{\ell}(\boldsymbol{\pi},\boldsymbol{\mu},\boldsymbol{\sigma}|\mathbf{x},\mathbf{z})
&=\sum_{i=1}^n\sum_{g=1}^Gz_{ig}\left\{\ln\left(\pi_g\right)+\ln\left(f_g(x_i|\mu_g,\sigma_g)\right)\right\}
\end{align*}

* Deutlich **einfacheres Maximierungsproblem**, da nun keine Summe innerhalb der Logarithmusfunktion. Den Logarithmus der Normalverteilung zu berechnen vereinfacht das Minimierungsproblem wunderbar, da die Normalverteilung zur Exponentialfamilie gehört. 

* Aber: Wir beobachten die Realisationen $\mathbf{z}$ nicht, sondern kennen lediglich die Verteilung der Zufallsvariablen $\mathbf{Z}=(Z_{11},\dots,Z_{nG})$. Dies führt zu einer stochastischen Version der Log-Likelihood Funktion:
$$
\tilde{\ell}(\boldsymbol{\pi},\boldsymbol{\mu},\boldsymbol{\sigma}|\mathbf{x},\mathbf{Z})=\sum_{i=1}^n\sum_{g=1}^GZ_{ig}\left\{\ln\left(\pi_g\right)+\ln\left(f_g(x_i|\mu_g,\sigma_g)\right)\right\}
$$
* Von dieser können jedoch den bedingten Erwartungswert berechnen:
$$
E(\tilde{\ell}(\boldsymbol{\pi},\boldsymbol{\mu},\boldsymbol{\sigma}|\mathbf{x},\mathbf{z})|X_i=x_i)=\sum_{i=1}^n\sum_{g=1}^Gp_{ig}\left\{\ln\left(\pi_g\right)+\ln\left(f_g(x_i|\mu_g,\sigma_g)\right)\right\}
$$


### Der EM-Algorithmus: Die abstrakte Version {-}

Der folgende EM-Algorithmus unterscheidet sich wieder lediglich in der Notation von den oben bereits besprochenen Versionen. Die folgende Notation verdeutlicht das Grundproblem der Maximierung der Log-Likelihood Funktion und ist abstrakt genug, um die Grundidee auf andere Maximum Likelihood Probleme zu übertragen: 
...
2. Für $r=1,2,\dots$
    ...
    &=\sum_{i=1}^n\sum_{k=1}^Kp_{ig}^{(r-1)}\left\{\ln\left(\pi_g\right)+\ln\left(\phi_{\mu_g\sigma_g}(x_i)\right)\right\}
    \end{align*}
    
    - <span style="color:#2471A3">**(Maximization)**</span>  Berechne:
    \begin{align*}
    \boldsymbol{\theta}^{(r)}=\arg\min_{\boldsymbol{\theta}}\mathcal{Q}(\boldsymbol{\theta},\boldsymbol{\theta}^{(r-1)})
    \end{align*}
