...
3. Prüfe Konvergenz 
  + **Entweder:** Stoppe falls sich die Parameterschätzer nicht mehr ändern.
  + **Oder:** Stoppe, falls sich der Wert der maximierten Log-Likelihood Funktion nicht mehr ändert.
...
Die Zuordnungen $z_{ig}$ können also die Werte $z_{ig}\in\{0,1\}$ annehmen, wobei $\sum_{g=1}^Gz_{ig}=1$ für alle $i=1,\dots,n$.

> **Beachte:**  Für jeden Datenpunkt $i$ (jeder Pinguin $i$) gibt es nur **eine** Gruppe (daher $\sum_{g=1}^Gz_{ig}=1$). Dies ist eine wichtige Restriktion von GMMs, welche bei den Pinguin-Daten unproblematisch ist, in anderen Anwendungen mit hirarchischen Gruppierungen aber evtl. problematisch ist. 

Die Zuordnungen $z_{ig}$ sind leider unbekannt (latent). Wir wissen aber trotzdem einiges etwas über diese Zuordnungen. Die Gewichte $\pi_1,\dots,\pi_G$ der Gaußschen Mischverteilung 
$$
f_G(x|\boldsymbol{\pi},\boldsymbol{\mu},\boldsymbol{\sigma})=\sum_{g=1}^G\pi_gf(x|\mu_g\sigma_g),
$$
geben die Anteile der Einzelverteilungen $f_g$ an der Gesamtverteilung $f$ an. Im Schnitt kommen $\pi_g\cdot 100\%$ der Datenpunkte von Verteilung $f_g$, $g=1,\dots,G$. Somit können wir die Zuordnungsdaten $\mathbf{z}=\big((z_{11},\dots,z_{1G}),\dots,(z_{n1},\dots,z_{nG})\big)$ als eine Realisation einer einfachen (i.i.d.) Zufallsstichprobe $(Z_1,\dots,Z_n)$ auffassen, wobei $Z_i\in\{(z_1,\dots,z_G)\in\{0,1\}^G\text{ so, dass }\sum_{g=1}^Gz_g=1\}$ eine diskrete Zufallsvariable ist. Die Wahrscheinlichkeitsfunktion $p_Z$ von $Z_i=(Z_{i1},\dots,Z_{iG})$ lautet:
$$
\begin{array}{ll}
...
Man bezeichnet die Wahrscheinlichkeiten $\pi_1,\dots,\pi_G$ als die **„a-priori-Wahrscheinlichkeiten“**. Wenn wir nichts über die Flossenlänge von Pinguin $i$ wissen, dann bleiben uns nur die a-priori-Wahrscheinlichkeiten: Mit Wahrscheinlichkeit $\pi_g$ gehört Pinguin $i$ zu Gruppe $g$. 


Falls wir die Flossenlänge von Pinguin $i$ erfahren, können wir die a-priori-Wahrscheinlichkeiten mit Hilfe des Satzes von Bayes aktualisieren. Dies führt dann zur **a-posteriori-Wahrscheinlichkeit $\;p_{ig}$:**
